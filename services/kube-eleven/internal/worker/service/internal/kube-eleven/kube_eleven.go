...
func (k *KubeEleven) generateFiles() error {
	template, err := tmplutils.LoadTemplate(templates.KubeOneTemplate)
	if err != nil {
	...
	err = tmplutils.Templates{Directory: k.outputDirectory}.Generate(template, generatedKubeoneManifestName, templateParameters)
	if err != nil {
