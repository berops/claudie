...
func (d *Deleter) DeleteNodes() error {
	...
	if err != nil {
		return fmt.Errorf("error while getting nodes from cluster %s : %w", d.clusterPrefix, err)
	}
	...
	for _, master := range d.masterNodes {
		...
		if err := d.deleteFromEtcd(kubectl, etcdEpNode); err != nil {
			return fmt.Errorf("error while deleting nodes from etcd for %s : %w", d.clusterPrefix, err)
		}
		// delete master nodes
		if err := d.deleteNodesByName(kubectl, master, realNodeNames); err != nil {
			return fmt.Errorf("error while deleting nodes from master nodes for %s : %w", d.clusterPrefix, err)
		}
	}
