...
func handlerInner(
	...
	var kuberWork Work
	{
		kuberWork.InputManifestName = inputManifestName
		kuberWork.WorkersLimit = workersLimit
		kuberWork.Task = work.Task
		work.Task = nil

		for _, pass := range work.Passes {
			var stage spec.StageKuber_SubPass
			...
			kuberWork.Passes = append(kuberWork.Passes, &stage)
		}
	...
	result := ProcessTask(ctx, kuberWork)
