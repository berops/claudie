...
//	Events            map[string]Events   `bson:"events"`
//}
//
...
//	}
//
//	events, err := ConvertFromGRPCEvents(config.Events)
//	if err != nil {
//		return fmt.Errorf("error while events converting %s from protobuf to db representation: %w", config.Name, err)
//	}
//
...
//		BuilderTTL:        int(config.GetBuilderTTL()),
//		SchedulerTTL:      int(config.GetSchedulerTTL()),
...
//}
//
//// UpdateTaskLease will update a builderTTL based on the name of the config
//// returns error if not successful, nil otherwise
//func (m *MongoDBConnector) UpdateTaskLease(name, cluster string, newLease int32) error {
//	return m.updateDocument(bson.M{"name": name}, bson.M{"$set": bson.M{
//		fmt.Sprintf("events.%s.lease", cluster): newLease,
//	}})
//}
//
...
//}
//
//func (c *MongoDBConnector) PushEvents(configName string, events map[string]*spec.Events) error {
//	if events == nil {
//		return nil
//	}
//
//	dbEvents, err := ConvertFromGRPCEvents(events)
//	if err != nil {
//		return err
//	}
//
//	return c.updateDocument(bson.M{"name": configName}, bson.M{
