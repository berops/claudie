...
func (u *Usecases) DestroyInfrastructure(ctx context.Context, request *pb.DestroyInfrastructureRequest) (*pb.DestroyInfrastructureResponse, error) {
	...
	// Concurrently destroy the infrastructure, OpenTofu state and state-lock files for each cluster
	err := concurrent.Exec(clusters, func(_ int, cluster Cluster) error {
		...
		// After the infrastructure is destroyed, we need to delete the OpenTofu state file from MinIO
		// and OpenTofu state-lock file from DynamoDB.
		...
		logger.Info().Msgf("Successfully deleted OpenTofu state and state-lock files for %q", cluster.Id())
		...
		// there are additional DNS related OpenTofu state and state-lock files.
		if _, ok := cluster.(*loadbalancer.LBcluster); ok {
			...
			logger.Info().Msg("Successfully deleted DNS related OpenTofu state and state-lock files")
