...
resource "azurerm_subnet" "{{ $nodepool.Name }}_{{ $clusterName }}_{{ $clusterHash }}_{{ $sanitisedRegion }}_{{ $specName }}_subnet" {
  ...
  virtual_network_name = azurerm_virtual_network.claudie_vn_{{ $specName }}_{{ $sanitisedRegion }}_{{ $clusterName }}_{{ $clusterHash }}.name
  address_prefixes     = ["{{ index $.Metadata (printf "%s-subnet-cidr" $nodepool.Name)  }}"]
}

resource "azurerm_subnet_network_security_group_association" "{{ $nodepool.Name }}_{{ $clusterName }}_{{ $clusterHash }}_{{ $sanitisedRegion }}_{{ $specName }}_associate_nsg" {
  provider                  = azurerm.nodepool_{{ $sanitisedRegion }}_{{ $specName }}
  subnet_id                 = azurerm_subnet.{{ $nodepool.Name }}_{{ $clusterName }}_{{ $clusterHash }}_{{ $sanitisedRegion }}_{{ $specName }}_subnet.id
  network_security_group_id = azurerm_network_security_group.claudie_nsg_{{ $specName }}_{{ $sanitisedRegion }}_{{ $clusterName }}_{{ $clusterHash }}.id
}
