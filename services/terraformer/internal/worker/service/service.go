...
var (
	...
	SpawnProcessLimit = envs.GetOrDefaultInt("TERRAFORMER_CONCURRENT_PROCESSES", 30)
...
func (s *Service) Stop() {
	...
	<-s.consumer.loopExited // we are no longer processing any messages, close all other connections.
...
func errHandler(consumeCtx jetstream.ConsumeContext, err error) {
	if errors.Is(err, nats.ErrConsumerDeleted) {
		log.
			Warn().
			Msgf("Received consumer error: %s0, closing down current consumer", err.Error())
