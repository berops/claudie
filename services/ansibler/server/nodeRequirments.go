...
func installLonghornRequirements(k8sNodepools []*NodepoolInfo) error {
	...
	if err != nil {
		return fmt.Errorf("failed to generate inventory file for all nodes : %w", err)
	}
	//run playbook
	...
	if err != nil {
		return fmt.Errorf("error while running ansible to install Longhorn requirements : %w", err)
	}
	//Clean up
