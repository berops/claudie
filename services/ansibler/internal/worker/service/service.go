...
var (
	...
	AckWait = time.Duration(envs.GetOrDefaultInt("ANSIBLER_ACK_WAIT_TIME", 8)) * time.Minute
)
...
func (s *Service) consumerLoop(exit chan<- struct{}) {
	...
	for {
		...
		case <-cctx.Closed():
			log.
				Info().
				Msg("Current consumer stopped, waiting for any pending processes to finish")
