...
func setUpNodeExporter(lbCluster *spec.LBcluster, clusterDirectory string, processLimit *semaphore.Weighted) error {
	...
	template, err := tmplutils.LoadTemplate(templates.NodeExporterPlaybookTemplate)
	if err != nil {
		return fmt.Errorf("error while loading %s template for node_exporter playbook : %w", lbCluster.ClusterInfo.Name, err)
	}

	tpl := tmplutils.Templates{Directory: clusterDirectory}
...
func setupEnvoyProxyViaDocker(
	...
	for _, tg := range targets {
		...
		tpl := tmplutils.Templates{Directory: dir}
	...
	compose, err := tmplutils.LoadTemplate(templates.EnvoyDockerCompose)
	if err != nil {
	...
	envoyPlayBook, err := tmplutils.LoadTemplate(templates.EnvoyTemplate)
	if err != nil {
