...
// TODO: move this validation to the operator.
// Check if the new config has reference to already existing static nodes.
//func validateStaticNodePools(ctx context.Context, newConfig *spec.Config, store store.Store) error {
//	allConfigs, err := store.ListConfigs(ctx)
//	if err != nil {
//		return fmt.Errorf("failed to retrieve all configs from database: %w", err)
//	}
//
//	var m manifest.Manifest
//	if err := yaml.Unmarshal([]byte(newConfig.Manifest.Raw), &m); err != nil {
//		return fmt.Errorf("config %q failed to unmarshal manifest: %w", newConfig.Name, err)
//	}
...
//			return fmt.Errorf("reference to the same static node with IP %q referenced in the newly added config %q is already in use by config %q, reference to the same static node across different clusters/configs is discouraged as it can lead to corrupt state of the cluster", match, newConfig.Name, cfg.Name)
//		}
//	}
//
//	return nil
